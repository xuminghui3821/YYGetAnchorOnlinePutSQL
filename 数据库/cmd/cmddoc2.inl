	...
	inline CStringA CDocumentImpl<T>::GetDocument(LONG nHashCode,int nFlags)
	{
		...
		if (DocFlags.IsDocument() ) {
			...
			if (Document.size()) {
				...
				//if (nHashCode != 0 && _T) {
				//	_TempFlags.SetEncryptRandXOR(true); 
				//}

				if (_TempFlags.IsEncrypt()) {
					int nCoder = Encrypt::HEX ;

					if (_TempFlags.IsBase64()) {
						nCoder = Encrypt::BASE64 ;
					}
					else if (_TempFlags.IsUTF8()) {
						nCoder = Encrypt::UTF8 ; 
					}
					else {
						_TempFlags.SetHEXCoder(true); 
					}
					...
					if (_TempFlags.IsHEX()) {
						std::string sCoder = HEX::HexEncode(json); 
						sDoc = sCoder.c_str(); 
					}
					else if (_TempFlags.IsBase64()) {
						std::string sCoder = Base64::Base64Encode(json); 
						sDoc = sCoder.c_str(); 
					}
					else if (_TempFlags.IsUTF8()) {
						StringHelper::CUTF8Encoder _Coder(json.c_str()); 
						sDoc = _Coder.m_pStr ; 
					}
					else {
						sDoc = json.c_str(); 
				...
				else if (_TempFlags.IsSignCRC32()) {
					// CRC32 签名
		...
		if (_Reader.parse(json,_Root))	{
			...
			if (Cmd.isMember("zdata")) {
				if (Flags.IsEncrypt()) {
					...
					if (Flags.IsBindMac()) {
						if (nHashCode == 0) {
							SetLastError(CMD_ERROR_FORMAT,"未设定有效的硬件绑定信息");
							goto _Exit;
						}
					...
					if (!(fSucceeded = _Reader.parse(sParameters,Document))) {
						SetLastError(CMD_ERROR_JSON,"无效返回数据，非JSON格式数据");
						Document.clear();
						goto _Exit;
					}
				}
					...
					if (Flags.IsHEX()) {
						...
						std::string json = HEX::HexDecode(Cmd["zdata"].asString()); 

						if (reader.parse(json, value)) {
							Document = value; 
						}
						else {
							SetLastError(CMD_ERROR_JSON,"无效返回数据，非JSON格式数据或解码错误");
							Document.clear(); 
							goto _Exit; 
						}
					}
					else if (Flags.IsBase64()) {
						Json::Reader reader; 
						Json::Value value ; 
						
						std::string json = Base64::Base64Decode(Cmd["zdata"].asString()); 
						...
						StringHelper::CUTF8Decoder _Coder(Cmd["zdata"].asString().c_str()); 
 						std::string json = _Coder.m_pStr ;
						...
						if (reader.parse(Cmd["zdata"].asString(), value)) {
							Document = value; 
						}
						else{
							Document = Cmd["zdata"];
			...
			SetLastError(CMD_ERROR_JSON,"无效数据包！非JSON数据");
		}
	_Exit:
		return Document;
	}
